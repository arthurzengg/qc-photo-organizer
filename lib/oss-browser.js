  ...
  function mount(opts) {
    ...
    function listLevel(prefix) {
      ...
      function page(marker, guard) {
        var q = { prefix: prefix, delimiter: '/', 'max-keys': 1000 };
        ...
        return client.list(q, {}).then(function (res) {
          ...
          (res.objects || []).forEach(function (ob) {
            if (ob.name === prefix) return;            // 跳过「文件夹占位对象」自身
            files.push({ key: ob.name, size: ob.size, lastModified: ob.lastModified });
          });
          if (res.isTruncated && res.nextMarker && guard < 20) return page(res.nextMarker, guard + 1);
        });
      }
    ...
    function render(data) {
      ...
      box.innerHTML =
        ...
        '</div>' +
        '<div class="ossb-stat">' + data.folders.length + ' 个文件夹 · ' + data.files.length + ' 个文件' +
          ' · 更新于 ' + esc(fmtTime(new Date())) + '</div>' +
    ...
    function load(showSpin) {
      ...
      return listLevel(cur).then(function (data) {
