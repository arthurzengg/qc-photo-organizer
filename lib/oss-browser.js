 ...
 *     deleteEndpoint: 'https://…/delete-record', // 可选:配了才出现「删除」按钮
 *     canDelete: true,                        // 可选:当前用户是否获授权(Harry/Arthur)
 *     currentUser: 'Harry'                    // 可选:随删除请求上送,供服务端审计
 *   });
 *
...
(function () {
  ...
  function mount(opts) {
    ...
    // 「删除」:经服务端函数删除该对象。前端只负责口令与二次确认;真正的
    // 权限校验在 Netlify 函数里(删除口令 + 用户白名单 + 路径白名单)。
    // 删除钥匙只在服务端,绝不下发到浏览器。
    function ensurePass() {
      var p = '';
      try { p = sessionStorage.getItem('qc_del_pass') || ''; } catch (e) {}
      if (!p) {
        p = window.prompt('请输入删除口令（仅 Harry / Arthur）');
        if (p == null) return null;                 // 用户取消
        p = String(p).trim();
        if (!p) return null;
        try { sessionStorage.setItem('qc_del_pass', p); } catch (e) {}
      }
      return p;
    }

    function doDelete(f, btn) {
      if (!f || !opts.deleteEndpoint) return;
      var pass = ensurePass();
      if (pass == null) { resetDel(btn); return; }   // 取消 → 复位按钮
      var prev = '删除';
      btn.disabled = true; btn.textContent = '删除中…'; btn.classList.remove('ossb-btn--delc');
      fetch(opts.deleteEndpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: f.key, user: opts.currentUser || '', passphrase: pass }),
      }).then(function (r) {
        return r.json().catch(function () { return {}; }).then(function (j) {
          return { ok: r.ok, status: r.status, j: j };
        });
      }).then(function (res) {
        if (res.ok) { load(true); return; }           // 成功 → 刷新列表(该行消失)
        if (res.status === 401) { try { sessionStorage.removeItem('qc_del_pass'); } catch (e) {} }
        btn.disabled = false; resetDel(btn); btn.textContent = prev;
        alert('删除失败：' + ((res.j && res.j.error) || ('HTTP ' + res.status)));
      }).catch(function (e) {
        btn.disabled = false; resetDel(btn); btn.textContent = prev;
    ...
    function armDel(btn) {
      ...
      btn.classList.add('ossb-btn--delc');
      setTimeout(function () { resetDel(btn); }, 4000);  // 4 秒内不确认则自动复位
    }
    function resetDel(btn) {
      if (!btn) return;
      btn.removeAttribute('data-armed');
      btn.classList.remove('ossb-btn--delc');
      if (btn.textContent === '确认删除') btn.textContent = '删除';
    }
    ...
    function render(data) {
      ...
      data.files.forEach(function (f, idx) {
        ...
        // 删除:仅在配置了删除端点且当前用户获授权(Harry/Arthur)时出现。
        if (opts.deleteEndpoint && opts.canDelete && /\.zip$/i.test(name)) {
          actions += '<button type="button" class="ossb-btn ossb-btn--del" data-ossdel="' + idx + '">删除</button>';
        }
    ...
    box.addEventListener('click', function (e) {
      ...
      var del = e.target.closest('[data-ossdel]');
      if (del) {
        e.preventDefault();
        if (del.getAttribute('data-armed') === '1') {   // 第二次点击 → 真正删除
          doDelete(renderedFiles[+del.getAttribute('data-ossdel')], del);
        } else {                                          // 第一次点击 → 进入确认态
          armDel(del);
        }
        return;
