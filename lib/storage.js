...
(function () {
  ...
  // body: Uint8Array preferred (no blob registry read on iOS WebKit — avoids
  // "The I/O read operation failed"); Blob accepted as fallback.
  function uploadZip(path, body) {
  ...
  // 存在数据库 folder 列里。
  function makeZipPath(folder, subfolder) {
    var date = new Date().toISOString().slice(0, 10);
    return 'records/qc-photo-organizer/' + (subfolder || '质检员首次检查') + '/' + date + '/' + folder + '.zip';
  }
  ...
  function upload(rec) {
    ...
    var zipPath = makeZipPath(rec.folder, rec.subfolder);
    return uploadZip(zipPath, rec.bytes || rec.blob).then(function () {
      return insertRecord({
        model: rec.model,
        unit: rec.unit,
