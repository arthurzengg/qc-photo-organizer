...
(function () {
  ...
  // fetch() handles Blob bodies natively in WebKit without going through FileReader.
  function putViaFetch(c, key, blob) {
    var ct = blob.type || 'application/octet-stream';
    // Include Content-Type in the signature so the signed URL matches what
    // the browser will send automatically from the Blob's type.
