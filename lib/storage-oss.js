...
(function () {
  ...
  // on some iOS/Android WebKit versions ("The 1/0 read operation failed").
  // Using a pre-signed PUT URL + fetch() bypasses that code path entirely.
  //
  // Additionally, iOS Safari fails to send a JSZip Blob directly as a fetch()
  // body ("Load failed"). Converting via blob.arrayBuffer() first — a native
  // Promise API that doesn't involve FileReader — fixes this on iOS 14.5+.
  function putViaFetch(c, key, blob) {
    var ct = blob.type || 'application/octet-stream';
    var url = c.signatureUrl(key, { method: 'PUT', expires: 300, 'Content-Type': ct });
    var bodyP = typeof blob.arrayBuffer === 'function'
      ? blob.arrayBuffer()
  ...
  //         defectSurfaceCount, attachCount, notes, subfolder }
  function upload(rec) {
    ...
    return putViaFetch(c, zipKey, rec.blob).then(function () {
      return putViaFetch(c, jsonKey, jsonBlob);
    }).then(function () {
